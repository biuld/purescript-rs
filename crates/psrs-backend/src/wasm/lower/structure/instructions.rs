        ...
        for instruction in instructions {
            match instruction {
                ...
                MirInstruction::LinearAllocDynamic {
                    ...
                    self.emit_linear_alloc_dynamic(body, *destination, *bytes, *alignment, *span)?
                }
                ...
                MirInstruction::LinearLoad {
                    ...
                    self.load(body, *address, *span)?;
                    body.push(Op::Leaf(linear_load(*ty, *offset, memory.0, *alignment)));
                    self.store(body, *destination, *span)?;
                }
                MirInstruction::LinearStore {
                    ...
                    self.load(body, *address, *span)?;
                    self.load(body, *value, *span)?;
                    body.push(Op::Leaf(linear_store(*ty, *offset, memory.0, *alignment)));
                }
                MirInstruction::LinearClosureNew { .. } => {
                    self.emit_linear_closure_new(body, instruction)?;
                }
                ...
                MirInstruction::LinearClosureGetCapture { .. } => {
                    self.emit_linear_closure_get_capture(body, instruction)?;
                }
