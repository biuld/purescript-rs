...
fn verify_array_map(
    ...
    if path_escapes_without_store(body_id, header_id, exit_id, &blocks, &stores) {
        return Err(mir_error(
            span,
...
fn path_escapes_without_store(
    ...
    exit: BlockId,
    blocks: &HashMap<BlockId, &crate::mir::BasicBlock>,
    stores: &HashSet<BlockId>,
) -> bool {
    let mut seen = HashSet::new();
    let mut pending = vec![start];
    while let Some(id) = pending.pop() {
        if id == header || id == exit {
            return true;
        }
        if stores.contains(&id) || !seen.insert(id) {
            continue;
        }
        if let Some(terminator) = blocks.get(&id).and_then(|block| block.terminator.as_ref()) {
            pending.extend(successors(terminator));
        }
    }
    false
