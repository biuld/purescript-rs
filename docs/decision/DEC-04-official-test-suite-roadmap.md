...
| BE-02 | Closure conversion, captures, direct calls, and closure calls | Top-level functions, local lambdas, `Int`/`Boolean`/`Number` scalar captures, concrete record/ADT captures, closure structs, and `call_ref` work. | Partial | Support polymorphic captures after frontend type-class and representation work. |
...
| BE-08 | Parameterized ADT representation and erasure | A first concrete slice boxes parameter-dependent `Int`/`Boolean`/`Number` fields as typed GC boxes and recovers references through `eqref`; fully polymorphic declarations remain rejected. | Partial | Generalize the erased layout and verify all instantiations. |
