...
| BE-11 | Strings, linear memory, data segments, and allocation | Target ([DEC-10](../../decision/DEC-10-canonical-abi-buffer-lifetime.md)): strings are GC values, linear memory is transient canonical scratch, `cabi_realloc` is a reclaiming allocator, and exports get `post-return`. Strings are now GC `(array (mut i16))` values: literals use passive segments and `array.new_data`, and the ABI adapter transcodes UTF-16 to the component's UTF-8. `cabi_realloc` is still a bump allocator with no reclamation, and no `post-return` is synthesized. P10 verifies the scratch region statically; dynamic reads rely on the Wasm bounds trap. [LM-01..LM-05](../implementation/backend/linear-memory-and-canonical-abi.md) and [ALC-01..ALC-07](../implementation/backend/canonical-buffer-allocation.md) are updated. | Partial | Replace the bump allocator with a reclaiming `cabi_realloc`, synthesize `post-return`, and broaden returned aggregate handling. |
...
| BE-19 | WIT aggregate values and resources | Resource handles and byte lists have a restricted path; closed WIT records with nested byte-list fields are classified and lowered in WIT field order. Indirect parameter tuples are allocated through `cabi_realloc`. Lists of strings, tuples, and general aggregates remain unsupported ([ABI-06](../implementation/backend/linear-memory-and-canonical-abi.md) In progress). | Partial | Add general aggregate layouts and resource ownership/lifetime rules. |
...
| BE-26 | Embedded standard library and user module loading | User modules are discovered from the entry files' directories and linked transitively ([WASI-09](../implementation/backend/wasi-platform.md) Verified); the PureScript-facing standard library is still embedded, not loaded from disk ([WASI-10](../implementation/backend/wasi-platform.md) In progress). | Partial | Load the standard library from disk and retire the embedded sources. |
...
| Data representation | BE-05..BE-10, BE-15 | Re-baselined by DEC-10: DR-03..DR-06 and DR-09..DR-12 are Verified; DR-01/DR-02 (string layout) and DR-07/DR-08 (string capture/box) are In progress for GC strings. | [DR-01..DR-12](../implementation/backend/data-representation.md) |
| Polymorphism and erasure | BE-02, BE-08; FE-09 input | Re-baselined by DEC-10: PE-01, PE-02, and PE-04..PE-11 are Verified; PE-03 (string box) and PE-08 (string capture) are In progress for GC strings. | [PE-01..PE-11](../implementation/backend/polymorphism-and-erasure.md) |
| Scalars and primitives | BE-04; FE-08 input | Re-baselined by DEC-10: SP-01 and SP-03..SP-12 are Verified; SP-02 (string representation) is In progress for GC strings. | [SP-01..SP-12](../implementation/backend/scalars-and-primitives.md) |
...
| Linear memory and canonical ABI | BE-11, BE-17..BE-20 | Re-baselined by DEC-10 and split: LM-05, ABI-01, ABI-06, and ABI-07 Verified; LM-01/LM-04 and ABI-02/ABI-03 In progress for GC strings, byte-list recovery, and handles; ABI-08 (aggregates, option/result/variant, tuples) Blocked on source types. The allocator, buffer free, and post-return moved to canonical buffer allocation. BE-19 stays `Partial`. | [LM/ABI rows](../implementation/backend/linear-memory-and-canonical-abi.md) |
| Canonical buffer allocation and lifetime | BE-11, BE-17..BE-20 | New topic split from linear memory by DEC-10: ALC-01..ALC-03 (allocator) and ALC-04/ALC-05 (buffer free) In progress; ALC-06 (`post-return`) Blocked because no current export returns a non-scalar; ALC-07 (dynamic-store provenance) In progress. | [ALC-01..ALC-07](../implementation/backend/canonical-buffer-allocation.md) |
| WASI platform and module loading | BE-21..BE-23, BE-26 | Re-baselined by DEC-10: WASI-01/02/04/06 and WASI-09 Verified; WASI-03 and WASI-05 In progress for GC strings/byte lists; WASI-07 Blocked on ABI-08; WASI-08 and WASI-10 remain. | [WASI rows](../implementation/backend/wasi-platform.md) |
...
| BC-03 | Linear memory and data-segment ABI | Passive data segments hold GC string literals; source `String` values are GC UTF-16 sequences transcoded at the boundary. The WASI `cabi_realloc` allocator and the canonical return area share one wasm32 memory. Reallocation checks alignment, address arithmetic, the old range and stored length prefix, and growth failure, and copies preserved bytes; there is no reclamation. Static MIR access-extent verification covers the scratch region. Linear memory is the canonical ABI boundary, not a language heap ([DEC-09](../decision/DEC-09-gc-only-language-heap.md)). | Partial | Replace the bump allocator with a reclaiming allocator, add the heap-state region, a target-selected pointer width, and broader ABI coverage. |
