...
| BE-11 | Strings, linear memory, data segments, and allocation | String literals use length-prefixed UTF-8 data; the bump `cabi_realloc` supports returned byte lists/strings, passing returned strings to another WIT import, and repeated allocations. It checks alignment, the old range and stored length prefix, address overflow, and growth failure, and copies preserved bytes on reallocation. P10 also performs static MIR access-extent verification (literal/scratch regions, read-only literals, wrapping-address propagation); dynamic reads rely on the Wasm bounds trap and dynamic stores are rejected. [LM-01..LM-05](../implementation/backend/linear-memory-and-canonical-abi.md) are Verified. | Partial | Define allocator ownership and reclamation, and broaden returned aggregate handling. |
...
| Data representation | BE-05..BE-10, BE-15 | Topic acceptance complete: DR-01..DR-12 have implementation, verifier, and required execution evidence. Broader feature rows retain their separate gates. | [DR-01..DR-12](../implementation/backend/data-representation.md) |
| Polymorphism and erasure | BE-02, BE-08; FE-09 input | Topic acceptance complete: PE-01..PE-11 have implementation, verifier, and required execution evidence. Broader feature rows retain their separate gates. | [PE-01..PE-11](../implementation/backend/polymorphism-and-erasure.md) |
| Scalars and primitives | BE-04; FE-08 input | Topic acceptance complete: SP-01..SP-12 have implementation, verifier, and required execution evidence. Broader feature rows retain their separate gates. | [SP-01..SP-12](../implementation/backend/scalars-and-primitives.md) |
...
| Linear memory and canonical ABI | BE-11, BE-17..BE-20 | In progress: LM-01..LM-05 and ABI-01..ABI-05/ABI-07 Verified; ABI-06 (general aggregate results, option/result/variant, non-byte lists, narrowed integers, tuples, own/borrow) remains specified but not produced. BE-19 stays `Partial`. | [LM/ABI rows](../implementation/backend/linear-memory-and-canonical-abi.md) |
| WASI platform and module loading | BE-21..BE-23, BE-26 | In progress: WASI-01..WASI-06 (console, clock, random, entry, componentization, per-service gating) and WASI-09 (user-module discovery) Verified; WASI-07 (filesystem/arguments/environment), WASI-08 (sockets/HTTP/TLS), and WASI-10 (standard library from disk) remain. | [WASI rows](../implementation/backend/wasi-platform.md) |
