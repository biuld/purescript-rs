...
| BE-19 | WIT aggregate values and resources | Resource handles lower with `own`/`borrow` drop rules; byte lists and closed WIT records with nested byte-list fields are classified and lowered in WIT field order. Indirect parameter tuples are allocated through `cabi_realloc`. Non-byte `list<T>` of scalars, `bool`, `char`, strings, and nullary enums is copied between a source GC array and the canonical buffer, with a driver execution test for `list<string>` ([ABI-08](../implementation/backend/linear-memory-and-canonical-abi.md) In progress). Tuples, `option`/`result`/`variant` payloads, and lists of aggregates remain unsupported; those WIT forms are not compiler source types ([DEC-11](../decision/DEC-11-primitive-ffi-stdlib-wrappers.md)). | Partial | Add general aggregate layouts beyond the list-and-handle subset. |
