...
| BE-11 | Strings, linear memory, data segments, and allocation | Target ([DEC-10](../../decision/DEC-10-canonical-abi-buffer-lifetime.md)): strings are GC values, linear memory is transient canonical scratch, `cabi_realloc` is a reclaiming allocator, and exports get `post-return`. Current code still uses length-prefixed linear strings and a bump allocator with no reclamation. P10 performs static MIR access-extent verification; dynamic reads rely on the Wasm bounds trap. [LM-01..LM-05](../implementation/backend/linear-memory-and-canonical-abi.md) are re-baselined. | Partial | Migrate to GC strings and the reclaiming allocator, and broaden returned aggregate handling. |
