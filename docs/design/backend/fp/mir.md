...
`Branch` omits `merge_block`: the structurer discovers joins from the graph
([control flow and tail calls](control-flow-and-tail-calls.md)). MIR and the
...
Current MIR emits `Return`, `Jump`, `Branch`, and `Switch`;
