...
| ABI-08 | General aggregate results, `option`/`result`/`variant` payloads, non-byte lists, tuples, and export `post-return` release lower or are rejected with named diagnostics. | Non-byte `list<T>` of scalars, `bool`, `char`, `string`/`list<u8>`, nullary enums, and directly flattened records of scalar fields is classified, validated, and lowered; `list<string>` has a driver execution test and the record element has a synthesized Wasm fixture. `option`/`result`/`variant`/tuple remain non-source types; records with string or nested fields, handles, and lists of those stay unsupported. | In progress |
